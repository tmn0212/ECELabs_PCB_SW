...
def write_bitstr_fast(bitstr='0'*20):
    ...
    D = {}
    PB0_7 = int(bitstr[0:8][::-1], 2)
    PB8_15 = int(bitstr[8:16][::-1], 2)
    PB16_19 = int(bitstr[16:20][::-1], 2)

    D["PB[0:7]"] = bitstr[0:8]
    D["PB[8:16]"] = bitstr[8:16]
    D["PB[16:19]"] = bitstr[16:20]
    write_allports(mcp_addr=0x20, value_portA=PB0_7, value_portB=PB8_15)
    write_allports(mcp_addr=0x21, value_portA=PB16_19, value_portB=0)
    # write_port(mcp_addr=0x21, port_addr=0x12, bit_str=PB16_19)
...
def read_all():
    ...
    mcp_x27 = [format(port, '08b') for port in read_allports(0x27)]
    LED = mcp_x27[portA][5:8] # LED = 0x26 portA {pin0=RED, pin1=GREEN, pin2=BLUE}
    BLUE = LED[0]   # this is string index, not pin #
    GREEN = LED[1]  # this is string index, not pin #
    RED = LED[2]    # this is string index, not pin #
    
    mcp_x26 = [format(port, '08b') for port in read_allports(0x26)]
    RIGHT = mcp_x26[portA]
    LEFT = mcp_x26[portB]
